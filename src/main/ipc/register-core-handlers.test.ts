...
const {
  ...
  registerGiteeHandlersMock: vi.fn(),
...
vi.mock('./gitee', () => ({
  registerGiteeHandlers: registerGiteeHandlersMock
}))
...
describe('registerCoreHandlers', () => {
  beforeEach(() => {
    ...
    registerGiteeHandlersMock.mockReset()
  ...
  it('passes the store through to handler registrars that need it', async () => {
    ...
    expect(registerGiteeHandlersMock).toHaveBeenCalledWith(store)
